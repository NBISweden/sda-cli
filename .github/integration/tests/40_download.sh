...
./sda-cli -config testing/s3cmd-download.conf download -pubkey user_key.pub.pem -dataset-id https://doi.example/ty009.sfrrss/600.45asasga -url http://localhost:8080 -outdir test-download main_folder/subfolder/dummy_data.c4gh
if [ ! -f "test-download/main_folder/subfolder/dummy_data.c4gh" ]; then
    echo "Downloaded file not found for first test"
    ls -r test-download
    exit 1
fi
C4GH_PASSWORD="" ./sda-cli decrypt -key user_key.sec.pem test-download/main_folder/subfolder/dummy_data.c4gh
# Check if file exists in the path
if [ ! -f "test-download/main_folder/subfolder/dummy_data" ]; then
    echo "Downloaded file not found for first test"
    exit 1
fi

# Check the first line of that file
first_line=$(head -n 1 test-download/main_folder/subfolder/dummy_data)
...
filepaths="download-dataset/main_folder/subfolder/dummy_data download-dataset/main_folder/subfolder2/dummy_data2 download-dataset/main_folder/subfolder2/random/dummy_data3"
...
./sda-cli -config testing/s3cmd-download.conf download -pubkey user_key.pub.pem -dataset-id https://doi.example/ty009.sfrrss/600.45asasga -url http://localhost:8080 -outdir download-folder --recursive main_folder/subfolder2

folderpaths="download-folder/main_folder/subfolder2/dummy_data2 download-folder/main_folder/subfolder2/random/dummy_data3"
...
# Check if file exists in the path
if [ ! -f "download-fileid/main_folder/subfolder/dummy_data.c4gh" ]; then
    echo "Downloaded file by using the file id not found"
    exit 1
fi

C4GH_PASSWORD="" ./sda-cli decrypt -key user_key.sec.pem download-fileid/main_folder/subfolder/dummy_data.c4gh
# Check the first line of the file
first_line_id=$(head -n 1 download-fileid/main_folder/subfolder/dummy_data)
...
content_paths="download-from-file/main_folder/subfolder/dummy_data.c4gh download-from-file/main_folder/subfolder2/dummy_data2.c4gh"
...
C4GH_PASSWORD="" ./sda-cli decrypt -key user_key.sec.pem download-from-file/main_folder/subfolder/dummy_data.c4gh
# Check the first line of the file
first_line_file=$(head -n 1 download-from-file/main_folder/subfolder/dummy_data)
