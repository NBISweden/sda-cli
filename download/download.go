...
func datasetCase(ctx context.Context, client downloadclient.Client) error {
...
func recursiveCase(ctx context.Context, client downloadclient.Client, args []string) error {
	...
	var allFiles []downloadclient.File
	...
	for _, dirPath := range dirPaths {
		var matched []downloadclient.File
		if apiVersionFlag == "v2" {
			files, err := client.ListFiles(ctx, datasetID, downloadclient.ListFilesOptions{PathPrefix: dirPath})
			if err != nil {
				return fmt.Errorf("failed to get files, reason: %v", err)
			}
			matched = files
...
func fileCase(ctx context.Context, client downloadclient.Client, args []string, fileList bool) error {
	var files []string
	if fileList {
...
// downloadOne fetches a single file via the downloadclient.Client and writes it
...
func downloadOne(ctx context.Context, client downloadclient.Client, userArg string) error {
	result, err := client.DownloadFile(ctx, downloadclient.DownloadRequest{
		DatasetID:       datasetID,
		UserArg:         userArg,
